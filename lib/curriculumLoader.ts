...
export class CurriculumLoader {
   ...
   */
  private static determineDifficulty(topicName: string, subcategory: string): 'Easy' | 'Medium' | 'Hard' {
    const easyKeywords = ['reading', 'writing', 'basic', 'simple', 'counting', 'comparing']
    const hardKeywords = ['percentage', 'rate', 'volume', 'angles', 'triangle', 'parallelogram', 'trapezium', 'composite']
    
    const topicLower = topicName.toLowerCase()
    const subcategoryLower = subcategory.toLowerCase()
    
    if (easyKeywords.some(keyword => topicLower.includes(keyword) || subcategoryLower.includes(keyword))) {
      return 'Easy'
    }
    
    if (hardKeywords.some(keyword => topicLower.includes(keyword) || subcategoryLower.includes(keyword))) {
      return 'Hard'
    }
   ...
   */
  private static determineProblemType(topicName: string, subcategory: string): 'addition' | 'subtraction' | 'multiplication' | 'division' | 'mixed' {
    const topicLower = topicName.toLowerCase()
    const subcategoryLower = subcategory.toLowerCase()
